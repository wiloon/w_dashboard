...
w_dashboard 是一个**纯本地原生桌面应用**，在每台主机上独立运行。它展示**本机**的状态：
...
5. 番茄钟（专注 / 休息计时，时段结束闪烁托盘/菜单栏图标，见 ADR-012 / SDD §11）
...
- 不要新增"非目标"功能（见 SDD §13）：跨机汇总、Web 端、历史/告警、多用户/鉴权、共享 core/FFI。
