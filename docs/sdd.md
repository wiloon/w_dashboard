...
```
                    ...
                    ┌──────────────▼─────┐     ┌─────▼──────────────┐
                    │     app-linux       │     │     app-macos       │
                    │  Rust + Iced/cosmic │     │   Swift + SwiftUI    │
                    │  ┌───────────────┐  │     │  ┌───────────────┐  │
...
├── app-linux/               # Rust + Iced/libcosmic：完整独立实现
...
# 可选：自动扫描根目录（默认空，不启用）。会递归查找其下的 .git 仓库。
[discovery]
scan_roots = []
# 扫描时忽略的目录名
ignore_dirs = ["node_modules", ".cache", "target"]
...
- **app-linux**：独立的 Rust 工程，`cargo build`；产物为可执行文件，配 `.desktop` 入口。测试 `cargo test`（含加载共享测试向量）。
